...
> **バージョン互換：** 本リリース（3.1.1）は **DSH ≥ 0.1.7-rc.1 かつ < 0.1.8** のみに対応。DSH 0.1.7 は命令的な設定登録（`settings.register` / `installSettingsSection`）とクライアントの `settingsScope` サービスおよびプラグイン別カードスロットを削除しました——旧ライン（3.0.x 以前）が依存する表面は存在しません。0.1.2–0.1.6 ホストではプラグイン 3.0.2 を使用してください。3.1.x は 0.1.7 の宣言的表面を対象とします：実行時に変更できる設定フィールドは schemastery スキーマで `.volatile()` 付きとし、設定フォームはホストがスキーマから自動生成（登録呼び出しもクライアント設定カードも不要）、プラグインは `loader/volatile-update` に従ってリクエストごとに最新値を読みます。

> **バージョン範囲ポリシー：** 各互換ラインはホストを自ラインのセグメントに厳密に固定します——`>=0.1.x-rc.1 <0.1.(x+1)-0`（3.1.x：`>=0.1.7-rc.1 <0.1.8-0`；3.0.x：`>=0.1.5-alpha.1 <0.1.6-0`；2.0.x：`>=0.1.2-alpha.1 <0.1.3-0`；1.0.0-beta：`>=0.1.0-rc.8 <0.1.2-alpha.1`）。どのラインも開放的な `<0.2.0-0` 上限を宣言しません。互換性レゾルバが未対応の新ホストセグメントにプラグインラインを誤マッチすることはありません。0.4.0–0.6.0 は dsh peer 宣言を一切持たないためインストールしないでください。
