 ...
 */
function scheduleEffort(history, allowDowngrade, allowUpgrade) {
    ...
    if (heaviest >= ESCALATION_ARGS && allowUpgrade)
        return 'max';
    if (history.failed)
        return HUB_EFFORT;
    if (calls.length < MIN_SAMPLE_FOR_DOWNGRADE)
        return HUB_EFFORT;
    if (simpleRatio(calls) >= 0.75 && allowDowngrade)
        return 'low';
    return HUB_EFFORT;
}
/**
