...
import type { ToolCallSample, ToolHistory } from './thinking-level.ts'
...
/**
 * Random-access view over a session's event log, whichever accessor the
 * installed harness exposes. `length` is the exclusive end of the log (dsh's
 * `session.seq`), so index `length - 1` is the most recent event.
 */
interface EventLog {
 ...
 */
function eventLogOf(session: Record<string, unknown>): EventLog | undefined {
  try {
    ...
    for (const accessor of ['snapshotEvents', 'ownEvents'] as const) {
      const read = session[accessor]
      if (typeof read !== 'function') continue
      const events = (read as () => unknown).call(session)
      if (Array.isArray(events)) return { length: events.length, at: index => events[index] }
    }
    // Pre-rename harness (0.1.2-alpha.*): a plain array member.
    const legacy = session.events
    if (Array.isArray(legacy)) return { length: legacy.length, at: index => legacy[index] }
  } catch {
 ...
 * than assumed absent.
 */
function isFailedToolResult(event: unknown): boolean {
  if (!isRecord(event) || event.type !== 'tool/result') return false
  const data = event.data
  if (!isRecord(data)) return false
  if (data.isError === true) return true
  const outcome = data.outcome
  if (isRecord(outcome) && (outcome.kind === 'error' || outcome.ok === false)) return true
 ...
 */
export function recentToolCalls(agent: unknown): ToolHistory | undefined {
  ...
  try {
    for (let index = log.length - 1; index >= 0; index -= 1) {
      ...
      if (isFailedToolResult(event)) failed = true
      ...
      if (calls.length >= TOOL_SAMPLE_WINDOW) break
    }
  } catch {
